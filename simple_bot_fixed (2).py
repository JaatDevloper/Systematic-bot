...
async def end_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if sorted_participants:
        ...
        for i, (user_id, data) in enumerate(sorted_participants[:max_participants]):
            ...
            # CRITICAL: Make sure negative_points is a number
            if negative_points is None:
                negative_points = 0
                
            adjusted_score = correct - negative_points
            
            # Calculate percentages based on actual number of questions answered
            actual_percentage = (correct / actual_questions_count) * 100 if actual_questions_count > 0 else 0
            adjusted_percentage = (adjusted_score / actual_questions_count) * 100 if actual_questions_count > 0 else 0
            
            # Format score display, always showing negative marking in the special format
            if negative_points > 0:
                score_text = f"{correct}-{negative_points:.2f}={adjusted_score:.2f}"
                results_message += f"{rank_emoji} {name}{username_text}: {score_text}/{actual_questions_count} ({adjusted_percentage:.1f}%)\n"
            else:
                # Standard display without negative marking
                results_message += f"{rank_emoji} {name}{username_text}: {correct}/{actual_questions_count} ({actual_percentage:.1f}%)\n"
