    ...
    if question_index < len(questions):
        ...
        if user_id not in active_quiz.get('participants', {}):
            active_quiz['participants'][user_id] = {
                ...
                'correct': 0,
                'answered': 0,
                'negative_points': 0  # CRITICAL: Always initialize negative_points
            }
...
async def end_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get all participants from quiz data - this is already prepared by poll_answer function
    participants = quiz.get('participants', {})
    logger.info(f"Existing participants: {participants}")
    
    # Get sent polls data for verifying question count
    sent_polls = quiz.get('sent_polls', {})
    ...
    for poll_id, poll_info in sent_polls.items():
        ...
        # For each poll, get the correct answer for negative marking
        correct_answer = poll_info.get('correct_option', 0)
        ...
        for user_id_str, answer_data in answers_data.items():
            ...
            if is_correct:
                ...
                if 'negative_points' not in participants[user_id]:
                    participants[user_id]['negative_points'] = 0
                participants[user_id]['negative_points'] += 0.25
                
            logger.info(f"Added/updated participant {user_id}: {participants[user_id]}")
    
    # Make sure all participants from the original quiz.participants are included
    # This helps ensure we don't miss anyone who participated
    original_participants = quiz.get('participants', {})
    for user_id, user_data in original_participants.items():
        # Convert to int if string
        user_id = int(user_id) if isinstance(user_id, str) else user_id
        
        # If this user is not in our reconstructed participants, add them
        if user_id not in participants:
            participants[user_id] = user_data
            logger.info(f"Added participant from original data: {user_id}")
    ...
    if not participants and 'creator' in quiz:
        ...
        if creator_id:
            ...
            participants[creator_id] = {
                ...
                'negative_points': 0  # Initialize negative points for creator as well
            }
    ...
    # Count the ACTUAL number of questions that were answered
    actual_questions_count = 0
    sent_polls = quiz.get('sent_polls', {})
    for poll_id, poll_info in sent_polls.items():
        # Count each question that was actually sent
        actual_questions_count += 1
    
    # If we have no polls, use the quiz questions length as fallback
    if actual_questions_count == 0:
        actual_questions_count = len(quiz.get('questions', []))
    
    logger.info(f"Actual questions count: {actual_questions_count}")
    
    # Create the results message
    results_message = f"🏁 The quiz has finished!\n\n{actual_questions_count} questions answered\n\n"
    
    # Make sure we have all connected users as participants (for multi-account scenario)
    # Get user list data and scan for all users who participated
    # Force all users to be shown in results
    all_users = {}
    
    # First, gather all answers from all polls
    for poll_id, poll_info in sent_polls.items():
        answers = poll_info.get('answers', {})
        for user_id_str, answer_data in answers.items():
            user_id = int(user_id_str) if isinstance(user_id_str, str) else user_id_str
            
            # Add user to our all_users tracking if not already there
            if user_id not in all_users:
                all_users[user_id] = {
                    'name': answer_data.get('user_name', f"Player {len(all_users)+1}"),
                    'username': answer_data.get('username', ''),
                    'correct': 0,
                    'answered': 0,
                    'negative_points': 0
            ...
            # Make sure they appear in participants too
            if user_id not in participants:
                participants[user_id] = {
                    'name': answer_data.get('user_name', f"Player {len(all_users)+1}"),
                    'username': answer_data.get('username', ''),
    ...
    # Always ensure there's a winner list shown
    if sorted_participants:
        ...
        # Make sure we use max of 50 participants to avoid message size limits
        max_participants = min(50, len(sorted_participants))
        
        for i, (user_id, data) in enumerate(sorted_participants[:max_participants]):
            # Use appropriate emoji for rankings
            rank_emoji = ["🥇", "🥈", "🥉"][i] if i < 3 else f"{i+1}."
            
            # Extract the correct user info
            ...
            # Ensure negative_points is properly initialized
            ...
            if actual_questions_count > 0:
                percentage = (adjusted_score / actual_questions_count) * 100
            else:
                percentage = 0
                
            # Format the score display differently depending on whether negative marking applies
            if negative_points > 0:
                # Show with negative marking calculation
                score_text = f"{correct}-{negative_points:.2f}={adjusted_score:.2f}"
                results_message += f"{rank_emoji} {name}{username_text}: {score_text}/{actual_questions_count} ({percentage:.1f}%)\n"
            else:
                # Show without negative marking calculation
                results_message += f"{rank_emoji} {name}{username_text}: {correct}/{actual_questions_count} ({percentage:.1f}%)\n"
